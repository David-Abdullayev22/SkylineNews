...
def get_news():
    base_url = 'https://new-science.ru'
    url = urljoin(base_url, '/category/news/page/2/')
    ...
    for index, item in enumerate(soup.find_all('h2', class_='post-title')):
        title = item.get_text()
        ...
        article_content_div = article_soup.find('div', class_='post-content')
        if not article_content_div:
            article_content_div = article_soup.find('div', class_='entry-content')
        if not article_content_div:
            article_content_div = article_soup.find('article')
        if not article_content_div:
        ...
        # Парсинг изображения
        img_tag = article_content_div.find('img')
        ...
        article_filename = f'articles/article_{index + 1}.html'
        ...
        content_tag = template_soup.new_tag('p')
    ...
    with open('science_news.html', 'w', encoding='utf-8') as file:
